...
for i in range(0, 28):
    ...
    # network에 넣기위해 전처리
    inpBlob \
        = cv2.dnn.blobFromImage(images[i], 1.0 / 255, (imageWidth, imageHeight), (0, 0, 0), swapRB=False, crop=False)
    ...
    H = output.shape[2]
    W = output.shape[3]
    print("이미지 ID : ", str(i), ", H : ", output.shape[2], ", W : ", output.shape[3])  # 이미지 ID
    ...
    for j in range(0, 9):  # 상체만: range(0, 9), 하체만: range(9, 16)
        ...
        if prob > 0.1:
            cv2.circle(images[i], (int(x), int(y)), 3, (0, 255, 255), thickness=-1,
                       lineType=cv2.FILLED)  # circle(그릴곳, 원의 중심, 반지름, 색)
            cv2.putText(images[i], "{}".format(j), (int(x), int(y)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1,
